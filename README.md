...
Il progetto Android nativo si trova in `android-app`. Le funzioni e la procedura di installazione sul Samsung Galaxy A36 sono descritte in [`docs/ANDROID.md`](docs/ANDROID.md). La build 0.13 può aggiungere al PNG esportato il logo trasparente, avviare dal telefono il flusso di pubblicazione OpenSea tramite il browser MetaMask, cambiare l'aspetto dell'interfaccia tramite nove temi persistenti e importare i temi JSON creati nel Pixel Sheet Theme Studio. La GitHub Action `Build Android APK` genera automaticamente un APK di collaudo installabile.
